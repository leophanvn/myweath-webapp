...
export const MODEL_NAME = 'ARTICLE';
export const MODEL_PLURAL = 'articles';
...
export const getListRss = async (payload = {}, next) => {
	return {
		type: SINGLE_API,
		payload: {
			url: `/${MODEL_PLURAL}/get-rss`,
			next,
		},
	};
};

export const getBySlug = async (payload = {}, next) => {
	const { filter, slug } = payload;

	return {
		type: SINGLE_API,
		payload: {
			url: `/${MODEL_PLURAL}/find-by-slug/${slug}${applyURIFilter(filter)}`,
			beforeCallType: 'GET_' + MODEL_NAME + '_DATA_REQUEST',
			successType: 'GET_' + MODEL_NAME + '_DATA_SUCCESS',
			next,
		},
